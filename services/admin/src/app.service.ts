  ...
  select: {
    ...
    const groupedChats = chats.reduce((acc, chat) => {
      ...
      if (!acc[chat.senderRole].orderIds.has(chat.orderId)) {
        ...
        if (chat.senderRole === 'admin') uniqueChat['adminId'] = chat.adminId;
      ...
      return acc;
    }, {} as Record<string, { chats: Array<{ orderId: number; requesterId?: number; adminId?: number; walkerId?: number }>, orderIds: Set<number> }>);
    ...
    const formattedGroupedChats = Object.keys(groupedChats).reduce((formattedAcc, role) => {
      ...
      return formattedAcc;
    }, {} as Record<string, Array<{ orderId: number; requesterId?: number; adminId?: number; walkerId?: number }>>);
